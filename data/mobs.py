...
from data.enums import Faction, RenderPriority
...
PLAYER_BODY = [Info(name="Player"), Player(), Actor(1, 1), Input(), AI(DeadPlayerController()), Renderable("%", [180,0,0], [0,0,0], RenderPriority.HIGH), Damageable(20, None, startHP=0, removeOnDeath=False), FOV(layout.LEVEL_WIDTH // 2), Light((255,0,0), lighting.MAX_LIGHT_LEVEL, False)]

PLAYER = [Info(name="Player", faction=Faction.PLAYER), Player(), Actor(10, 5, startingInitiative=0), Inventory(2), Equipment(), Input(), AI(PlayerController()), Damageable(20, PLAYER_BODY), Melee(3), Blocking(), FOV(layout.LEVEL_WIDTH // 2), Renderable("☻", [255,0,0], [0,0,0], RenderPriority.HIGH), Light((255,255,0), 8, False)]
